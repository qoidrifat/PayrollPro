...
import { EMPLOYEES, POSITIONS, DEPARTMENTS } from '../services/mockData';
...
export const EmployeeList: React.FC<Props> = ({ user, showNotification }) => {
  ...
  const handleSavePayroll = (data: any) => {
      ...
      // Simulate Backend Email Trigger
  ...
  if (showPayrollForm && selectedEmployee) {
      ...
      if (emp) return <PayrollForm employee={emp} onSave={handleSavePayroll} onCancel={() => setShowPayrollForm(false)} />;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            ...
            {user.role === Role.ADMIN && (
                <button className="bg-brand-blue text-white px-5 py-2.5 rounded-lg shadow hover:bg-blue-800 text-sm font-bold whitespace-nowrap transition-colors flex items-center justify-center">
                ...
                <tr key={emp.id} className="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors group">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex justify-end space-x-3 opacity-0 group-hover:opacity-100 transition-opacity">
                             ...
                             {user.role === Role.ADMIN && (
                                <button className="text-gray-400 hover:text-brand-blue dark:hover:text-blue-400 p-1 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700">
      ...
      <div className="md:hidden space-y-4">
        {employees.map((emp) => (
            <div key={emp.id} className="bg-white dark:bg-brand-dark p-5 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4 border-t border-gray-100 dark:border-gray-700 pt-4">
                    <div>
                        <p className="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider">Jabatan</p>
                        <p className="text-sm font-medium text-gray-900 dark:text-white">{getPositionName(emp.positionId)}</p>
                        <p className="text-xs text-brand-blue dark:text-blue-400 mt-1">{getDepartmentName(emp.departmentId)}</p>
                    </div>
                    <div className="text-right">
                        <p className="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider">Gaji Pokok</p>
