...
export const Layout: React.FC<LayoutProps> = ({ 
  ...
  const handleNavClick = (id: string) => {
      ...
      // 1. Trigger Loading Screen immediately
      setIsLoading(true);

      // 2. Move the sidebar box visually
      setVisualActiveTab(id);
      
      // 3. Close mobile menu if open
      setIsMobileMenuOpen(false);

      // 4. Delay the actual content switch to let animations play and simulate load
      // Sidebar animation is 1s. We wait 800ms to switch content, then fade out loader.
      setTimeout(() => {
          setActiveTab(id);
          // Loader will be turned off by parent (App.tsx) or we can do it here?
          // To be safe and allow React to render the new tab, we wait a tiny bit more
          setTimeout(() => {
            setIsLoading(false);
          }, 500);
      }, 800); 
